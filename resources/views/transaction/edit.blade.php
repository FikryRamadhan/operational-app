			...
			<div class="card-body">
				<form id="form">
					...
					<div class="form-group">
						...
						<textarea class="form-control" name="description" placeholder="Masukkan deskripsi" rows="2">{{ $transaction->description }}</textarea>
						<span class="invalid-feedback"></span>
					</div>
					...
					<div class="form-group">
						...
						<div class="input-group">
							...
							<input type="number" name="nominal" class="form-control" placeholder="Masukkan nominal transaksi" value="{{ $transaction->nominal > 0 ? $transaction->nominal : $transaction->nominal * -1 }}">
						</div>
						<span class="invalid-feedback"></span>
					</div>
...
<script>
	...
	$(function(){
		...
		$form.find(`[name="type"]`).val(`{{ $transaction->type }}`).trigger('change')
		$form.find(`[name="id_transaction_group"]`).val(`{{ $transaction->id_transaction_group }}`).trigger('change')
		setTimeout(() => {
			$form.find(`[name="id_category"]`).val(`{{ $transaction->id_category }}`).trigger('change')
		}, 1000)
