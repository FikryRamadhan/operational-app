			...
			<div class="card-body">
				<form id="form">
					...
					<div class="form-group">
						...
						<input type="text" name="phone_number" class="form-control" placeholder="62xxxxxxx" required>
						<span class="invalid-feedback"></span>
					</div>

					<div class="form-group">
						<label> Catatan (Opsional) </label>
						<textarea class="form-control" name="notes" placeholder="Catatan (Opsional"></textarea>
						<span class="invalid-feedback"></span>
					</div>
...
<script>
	...
	$(function(){
		...
		$form.on('submit', function(e){
			...
			ajaxSetup();
			$.ajax({
				url: `{{ route('report_receiver.store') }}`,
				method: 'post',
				data: formData,
				dataType: 'json',
			})
		...
		const resetForm = () => {
			...
			$form.find(`#transaction-group-table`).find('tbody').empty()
